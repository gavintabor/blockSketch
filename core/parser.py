...
def _parse_blocks(ts: _TokenStream, mesh: BlockMesh) -> None:
    ...
    while ts.peek() not in (None, ')'):
        ...
        if vids is None or len(vids) != 8:
            ...
            ts.skip_to(')', 'hex')
            continue
        # optional zone name between vertex list and cell counts
        zone_name = ''
        peek = ts.peek()
        if peek and peek != '(' and not peek[0].isdigit():
            zone_name = ts.next()
        # cell counts
    ...
    while ts.peek() not in (None, ')'):
        ...
        if kind in ('arc',):
            ...
            if None not in (v0, v1) and pt is not None:
                mesh.edges.append(
                    ArcEdge(v_start=v0, v_end=v1, point=pt, is_origin=is_origin))
            else:
                mesh.warnings.append("edges: malformed arc entry")
...
def _parse_default_patch(ts: _TokenStream, mesh: BlockMesh) -> None:
    if ts.peek() != '{':
        return
    ts.next()  # '{'
    dp_name = 'defaultPatch'
    dp_type = 'patch'
    while ts.peek() not in (None, '}'):
        key = ts.next()
        if key == 'name':
    ...
    if ts.peek() == ';':
        ts.next()
    mesh.default_patch_name = dp_name
    mesh.default_patch_type = dp_type
...
_KNOWN_KEYWORDS = frozenset({
    ...
    'geometry', 'defaultPatch', 'mergePatchPairs', 'mergeType', 'mergedMeshes',
})
...
def parse(text: str) -> BlockMesh:
    ...
    while ts.peek() is not None:
        ...
        elif kw == 'defaultPatch' and 'defaultPatch' not in seen:
            seen.add('defaultPatch')
            _parse_default_patch(ts, mesh)

        elif kw == 'mergePatchPairs' and 'mergePatchPairs' not in seen:
            seen.add('mergePatchPairs')
            if ts.peek() == '(':
                ts.next()
                while ts.peek() == '(':
                    ts.next()  # inner '('
                    ...
                    if ts.peek() == ')':
                        ts.next()
                    if p1 and p2:
                        mesh.merge_patch_pairs.append((p1, p2))
                if ts.peek() == ')':
                    ts.next()
            if ts.peek() == ';':
                ts.next()
