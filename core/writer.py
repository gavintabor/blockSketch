...
from core.model import ArcEdge, Block, BlockMesh, BoundaryPatch, ProjectEdge, SplineEdge
...
def _write_blocks(mesh: BlockMesh) -> str:
    ...
    for block in mesh.blocks:
        ...
        if block.zone_name:
            lines.append(f'    hex ({vids}) {block.zone_name} ({nx} {ny} {nz}) {grading}')
        else:
...
def _write_edges(mesh: BlockMesh) -> str:
    ...
    for edge in mesh.edges:
        if isinstance(edge, ArcEdge):
            if edge.is_origin:
                lines.append(
                    f'    arc {edge.v_start} {edge.v_end} origin ({_xyz(edge.point)})')
            else:
                lines.append(
...
def _write_default_patch(mesh: BlockMesh) -> str | None:
    if not mesh.default_patch_name:
        return None
    return (
        'defaultPatch\n'
        '{\n'
        f'    name  {mesh.default_patch_name};\n'
        f'    type  {mesh.default_patch_type};\n'
        '}'
    )


def _write_merge_patch_pairs(mesh: BlockMesh) -> str | None:
    if not mesh.merge_patch_pairs:
        return None
    lines = ['mergePatchPairs', '(']
    for p1, p2 in mesh.merge_patch_pairs:
...
def write_blockmesh(mesh: BlockMesh) -> str:
    ...
    dp = _write_default_patch(mesh)
    if dp:
        sections += [dp, '']
    mpp = _write_merge_patch_pairs(mesh)
    if mpp:
        sections += [mpp, '']
    sections += [div, '']
    return '\n'.join(sections)
