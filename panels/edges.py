...
class _EdgeItem(QFrame):
    ...
    def _build_ui(self) -> None:
        ...
        note = QLabel('Arc is defined by a single midpoint.')
        note.setStyleSheet('color: #999; font-style: italic; font-size: 11px;')
        arc_layout.addWidget(note)

        mid_row = QHBoxLayout()
        mid_lbl = QLabel('midpoint')
        mid_lbl.setStyleSheet('color: #555; font-size: 11px;')
        mid_lbl.setFixedWidth(55)
    ...
    def get_edge(self):
        ...
        if typ == 'arc':
            ...
            return ArcEdge(v_start=vs, v_end=ve, point=mid)
        else:
            pts = []
